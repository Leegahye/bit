...
public class DispatcherServlet extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    // getServletPath()의 리턴 값: /score/step05/list.do->서블릿 경로
    String pageUrl = request.getServletPath();
    // 2. pageUrl로 ServletContext로부터 페이지 컨트롤러를 꺼낸다.
    PageController pageController = //객체주소를 리턴받는 참조변수
    ...
    try {
      ...
      String viewUrl = pageController.execute(paramMap, model);
      								//웹브라우저의 요청값, 페이지 컨트롤러가 작업한 결과를 담는 빈 바구니
      
      // 5. 페이지 컨트롤러가 작업한 결과물을 ServletRequest 보관함에 담는다.
      for (Entry<String,Object> item : model.entrySet()) { //모델에 담긴 키값을 
        request.setAttribute(item.getKey(), item.getValue()); //request에 복사한 다음 jsp에 보낸다.
      }
      ...
      RequestDispatcher rd = request.getRequestDispatcher("/score/step05HW/error");
