...
public class Calculator extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		//POST명령으로 들어온 한글 데이터는 getParameter()를 호출하기 전에 다음 메서드를 먼저 호출하라
		request.setCharacterEncoding("UTF-8");

		//HttpServletRequest : 요청 정보를 다룰 때 사용할 도구가 들어있음.
		int num1 = Integer.parseInt(request.getParameter("num1"));
		int num2 = Integer.parseInt(request.getParameter("num2"));
		String cal = request.getParameter("value");
		...
		if(cal.equals("+")){
			out.println(num1+" + "+num2+" = "+ (num1+num2) + "입니다.");
		}else if(cal.equals("-")){
			out.println(num1+" - "+num2+" = "+ (num1-num2) + "입니다.");
		}else if(cal.equals("*")){
			out.println(num1+" + "+num2+" = "+ (num1*num2) + "입니다.");
		}else if(cal.equals("/")){
			out.println(num1+" + "+num2+" = "+ (double)(num1/num2) + "입니다.");
		}
	}
}
