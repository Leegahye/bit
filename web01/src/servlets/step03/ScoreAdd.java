...
public class ScoreAdd extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      //2)Refresh 정보를 HTML <head>태그에 넣는다.
      ...
      //3)웹 브라우저에게 재요청 정보를 전달한다.
      // => 본문은 보내지 않는다. => 캐시에 보관된 내용물을 버린다.
      // => 만약 캐시가 꽉 차서 sendRedirect()를 호출하기 전에 이미 일부내용을 보냈다면?
      //	sendRedirect()할 수 없다.   
  	  /* PrintWriter 객체를 통해 출력하면 출력내용은 즉시 웹 브라우저로 보내지지 않는다.
  	   * 출력 효율을 위해 => 임시 메모리(캐시)에 보관하였다가 캐시가 꽉 차면 비로소 출력한다.
  	   * 또는 베서드 호출이 끝나면 출력한다.
