...
-- 우편번호정보
DROP TABLE IF EXISTS POST_ADDRS RESTRICT;
...
CREATE TABLE SUB_REQS (
	...
	PAY_METHOD VARCHAR(50) NULL     COMMENT '결제방법', -- 결제방법
...
CREATE TABLE TEACHERS (
	...
	POSITION VARCHAR(50) NULL     COMMENT '직책' -- 직책
)
COMMENT '강사';
...
CREATE TABLE STUDENTS (
	MNO       INTEGER      NOT NULL COMMENT '수강생일련번호', -- 수강생일련번호
	POSTNO    VARCHAR(20)  NULL     COMMENT '우편번호', -- 우편번호
	...
	COMPANY   VARCHAR(50)  NULL     COMMENT '회사', -- 회사
...
CREATE TABLE SUBJECTS (
	...
	CLASS    VARCHAR(50)  NULL     COMMENT '교실', -- 교실
	CAPACITY INTEGER      NOT NULL COMMENT '수강가능인원', -- 수강가능인원
	DESC     TEXT         NOT NULL COMMENT '과목설명', -- 과목설명
...
-- 우편번호정보
CREATE TABLE POST_ADDRS (
	POST_NO  VARCHAR(20)  NOT NULL COMMENT '우편번호', -- 우편번호
	BAS_ADDR VARCHAR(255) NOT NULL COMMENT '기본주소' -- 기본주소
)
COMMENT '우편번호정보';

-- 우편번호정보
ALTER TABLE POST_ADDRS
	ADD CONSTRAINT PK_POST_ADDRS -- 우편번호정보 기본키
		PRIMARY KEY (
			POST_NO -- 우편번호
		);
...
-- 수강생
ALTER TABLE STUDENTS
	ADD CONSTRAINT FK_POST_ADDRS_TO_STUDENTS -- 우편번호정보 -> 수강생
		FOREIGN KEY (
			POSTNO -- 우편번호
		)
		REFERENCES POST_ADDRS ( -- 우편번호정보
			POST_NO -- 우편번호
		);
