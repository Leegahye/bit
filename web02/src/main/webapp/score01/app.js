...
function changeFormState(state){
		...
		for(var i = 0; i < elements.length; i++){
		  if(state == "updateState"){
			elements.item(i).style.display = "";//화면에출력/
		  }else{
...
function loadScoreDetail(event){
	event.preventDefault(); //웹 브라우저야, a 태그를 클릭할 때 수행하는 기본 작업을 하지말아라!
    ...
    if (xhr.readyState == 4) {
      // JSON 형식의 문자열을 실행하여 자바스크립트 객체를 생성한다. 
	   var score = JSON.parse(xhr.responseText); 
		...
		document.getElementById("execDate").value = new Date(score.execDate).toString("yyyy-MM-dd");
  ...
  xhr.onreadystatechange = function(event) {
	if (xhr.readyState == 4) {
	  ...
	  for (var i in scores) {
		...
		a.href = 'update.json?no=' + scores[i].no;
